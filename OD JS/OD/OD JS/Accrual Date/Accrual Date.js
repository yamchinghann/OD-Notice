...
function isMCExisted() {
    ...
    let MCItems = getSubFormData("SubFormMCInfo");
    return (MCItems.length > 0)
}

//Calculate Function
function sortedMCEndDateByDesc() {
    return getSubFormData("SubFormMCInfo").sort((a, b) => {
        return new Date(b.EndDateHUSInfo) - new Date(a.EndDateHUSInfo);
    });
}
...
function calculateAccrualDate() {
    ...
    if (isMCExisted() === false && !currentAccrualDate) {
        ...
        let MCs = sortedMCEndDateByDesc().filter((item) => {
            if (item.HUSTypeDropdown === "MC" && item.HUSApprovalStatusDisplayHUSDetailsHUSI.trim() === "Approved") {
                return item;
            }
        });
        let noticeDate = new Date(getFormFieldValue("NoticeandBenefitClaimFormReceivedDate1"));
        let lastMCDate = new Date(MCs[0].EndDateHUSInfo);
